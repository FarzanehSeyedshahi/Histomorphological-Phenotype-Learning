...
def get_aucs(model, data, label):
	...
	train_auc = average_precision_score(y_true=list(train_labels[:,label]), y_score=list(train_pred))

	aucs = [train_auc]
	valid_auc = None
	if valid is not None:
		# valid_auc = roc_auc_score(y_true=list(valid_labels[:,label]), y_score=list(valid_pred))
		print('valid report:', classification_report(list(valid_labels[:,label]), list(valid_pred), labels=[0,1]))
		valid_auc = average_precision_score(y_true=list(valid_labels[:,label]), y_score=list(valid_pred))
		aucs.append(valid_auc)

	# test_auc  = roc_auc_score(y_true=list(test_labels[:,label]),  y_score=list(test_pred))
	test_auc  = average_precision_score(y_true=list(test_labels[:,label]),  y_score=list(test_pred))
	print('test report:', classification_report(list(test_labels[:,label]), list(test_pred), labels=[0,1]))
	aucs.append(test_auc)
	additional_auc = None
	if additional is not None:
		additional_pred = model.predict(exog=additional_data)
		# additional_auc  = roc_auc_score(y_true=list(additional_labels[:,label]),   y_score=list(additional_pred))
		additional_auc  = average_precision_score(y_true=list(additional_labels[:,label]),   y_score=list(additional_pred))
...
def include_coefficients(model, frame_clusters_orig, features, label, groupby):
	...
	# frame_clusters['predictions_%s' % label] = model.predict(exog=frame_clusters[features])
	...
	# 	frame_clusters['%s_%s' % (column, label)] = [np.inf]*frame_clusters.shape[0]
	# 	for i, cluster_id in enumerate(features):
	# 		frame_clusters.loc[frame_clusters[groupby]==cluster_id, '%s_%s' % (column, label)] = coef[i]
...
def run_logistic_regression(alphas, resolutions, meta_folder, meta_field, matching_field, folds_pickle, h5_complete_path, h5_additional_path, force_fold, additional_as_fold, diversity_key=None,
	...
	for alpha in alphas:
		...
		# Save to CSV.
		columns = ['Leiden Resolution', 'Fold', 'Train f1']
		if valid_df is not None: columns.append('Valid f1')
