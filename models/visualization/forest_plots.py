...
class EffectMeasurePlot_LR:
    ...
    def plot(self, figsize=(3, 3), t_adjuster=0.01, decimal=3, size=3, max_value=None, min_value=None, fontsize=12, p_th=0.05):
        ...
        # Add to_numpy to df columns to fix error
        plot.errorbar(self.df.OR2.to_numpy(), self.df.index.to_numpy(), xerr=[self.df.LCL_dif, self.df.UCL_dif], marker='None', zorder=2, ecolor=self.errc, elinewidth=(size / size), linewidth=0)
        plot.scatter(self.df.OR2.to_numpy(), self.df.index.to_numpy(), c=self.pc, s=(size * 25), marker=self.shape, zorder=3, edgecolors='None')
